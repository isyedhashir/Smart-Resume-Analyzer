...
def decode_if_bytes(item):
    """Decode an item if it's a bytes type."""
...
def run():
    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'hashir' and ad_password == '1234':
                st.success("Welcome Hashir !")
                # Display Data
                cursor.execute('''SELECT*FROM user_data''')
                data = cursor.fetchall()
                data = [[decode_if_bytes(item) for item in row] for row in data]
                ...
                labels = [label.decode('utf-8') if isinstance(label, bytes) else label for label in plot_data.Predicted_Field.unique()]
                print(labels)
                ...
                labels = [label.decode('utf-8') if isinstance(label, bytes) else label for label in plot_data.User_level.unique()]
